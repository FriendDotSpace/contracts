...
interface IFriendKey is IERC1155 {
    enum RoomTier { Casual, Club, Exclusive }
    enum RoomType { Trading, Social }
    ...
    function registerCreator(RoomTier tier, uint256 additionalKeys, string calldata metadata, bytes calldata signature) external returns (uint256);
    function registerCreator(string calldata metadata, bytes calldata signature) external returns (uint256);
    function registerSocialCreator(RoomTier tier, uint256 additionalKeys, string calldata metadata, bytes calldata signature) external returns (uint256);
