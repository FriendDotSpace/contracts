 ...
 * @custom:oz-upgrades-from src/FriendPoolV2.sol:FriendPoolV2
 ...
 */
contract FriendPoolV3 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    /**
     * @notice Transfers a partial amount of funds from pool reserves to a room destination, deducting a topup fee
     * @dev Callable only by dispatcher or owner. The amount must be less than or equal to available reserves.
     *      Deducts topupFeeAmount and sends it to dev destination, then transfers the remaining net amount to destination.
     * @param tokenId The token ID whose reserves to transfer from
     * @param amount The amount of bonding tokens to transfer (must be <= poolReserves[tokenId])
     * @param topupFeeAmount The fee amount to deduct and send to dev destination
     * @param destination The address to receive the net amount (amount - topupFeeAmount)
     */
    function transferFundsPartialyToRoom(uint256 tokenId, uint256 amount, uint256 topupFeeAmount, address destination)
        external
        whenNotPaused
    {
        if (msg.sender != _dispatcher && msg.sender != owner()) revert Errors.CallerNotAuthorityOrOwner();

        uint256 available = poolReserves[tokenId];
        if (available == 0) revert Errors.NoFundsAvailable();
        if (amount == 0) revert Errors.AmountMustBeGreaterThanZero();
        if (amount > available) revert Errors.InsufficientReserves();
        if (amount <= topupFeeAmount) revert Errors.InsufficientReserves();
        if (destination == address(0)) revert Errors.ZeroAddress();
