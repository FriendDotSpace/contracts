 ...
 */
/// @custom:oz-upgrades-from FriendPoolV2
contract FriendPoolV3 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function dispatchAll(uint256 tokenId, address to, bytes32 orderId, bytes calldata data)
        external
        payable
        returns (uint256)
    {
        require(msg.sender == _dispatcher || msg.sender == owner(), "FriendPool: Caller is not the dispatcher");
        require(to == address(dlnSource), "FriendPool: Recipient must be DLN Source");
    ...
    function emergencyWithdraw(uint256 tokenId) external onlyOwner {
        uint256 amount = poolReserves[tokenId];
        require(amount > 0, "FriendPool: No funds available for withdrawal");

        IERC20Metadata bondingToken = IERC20Metadata(friendKey.bondingToken());
        require(bondingToken.balanceOf(address(this)) >= amount, "FriendPool: Insufficient pool reserves");

        poolReserves[tokenId] = 0;

        bondingToken.safeTransfer(owner(), amount);
        emit FundsDispatched(tokenId, amount, bytes32(0));
    }
    ...
    function donate(uint256 tokenId, uint256 amount) external {
        IERC20Metadata bondingToken = IERC20Metadata(friendKey.bondingToken());
        require(bondingToken.balanceOf(msg.sender) >= amount, "FriendPool: Insufficient bonding token balance");
        require(
            bondingToken.allowance(msg.sender, address(this)) >= amount,
