...
import {FriendStake} from "../FriendStakeV2.sol";
    ...
    UUPSUpgradeable
{
    ...
    function setFriendStake(address _friendStake) public onlyOwner {
        require(_friendStake != address(0), "FriendStake address cannot be zero");
        friendStake = _friendStake;
    }
    ...
    function registerCreator(RoomTier tier, uint256 additionalKeys) public returns (uint256) {
        ...
        FriendStake(cloneAddress).initialize(owner(), address(this), address(bondingToken), id, authority);
